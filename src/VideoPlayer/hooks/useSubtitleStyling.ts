    ...
    const applySubtitleStyles = () => {
      ...
      const styles = `
        .video-player video::cue {
          ...
          background: ${config?.backgroundColor || 'linear-gradient(135deg, #fbbf24, #f59e0b)'} !important;
          ...
          ${config?.position === 'top' ? 'top: 10% !important; bottom: auto !important;' : ''}
          ${config?.position === 'center' ? 'top: 50% !important; bottom: auto !important; transform: translateX(-50%) translateY(-50%) !important;' : ''}
          ${config?.position === 'bottom' || !config?.position ? 'bottom: 15% !important; top: auto !important;' : ''}
        }
      `;
